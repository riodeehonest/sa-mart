  ...
  if (step === 'success') {
    return (
      <div style={{ fontFamily: 'sans-serif', minHeight: '100vh', background: '#f5f5f5', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        ...
        <div style={{ fontSize: 14, color: '#999', marginBottom: 4 }}>Order #QM{Math.floor(Math.random() * 9000 + 1000)}</div>
        ...
        {['Cart', 'Checkout', 'Done'].map((s, i) => (
          <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{
              ...
              background: (step === 'cart' && i === 0) || (step === 'checkout' && i === 1) || (step === 'success' && i === 2) ? '#ff6b35' : i < (['cart', 'checkout', 'success'].indexOf(step)) ? '#4CAF50' : '#eee',
              color: (step === 'cart' && i === 0) || (step === 'checkout' && i === 1) ? 'white' : i < (['cart', 'checkout', 'success'].indexOf(step)) ? 'white' : '#999'
            }}>{i + 1}</div>
        ...
        <>
          ...
          <div style={{ background: 'white', margin: '0 16px 16px', borderRadius: 16, padding: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            {[
              ...
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#666', marginBottom: 8 }}>
                <span>{row.label}</span>
                <span style={{ color: deliveryFee === 0 && i === 1 ? '#4CAF50' : '#333', fontWeight: 500 }}>{row.value}</span>
              </div>
            ))}
      ...
      {step === 'checkout' && (
        <div style={{ padding: 16 }}>
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: 16, marginBottom: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            {addresses.map(addr => (
              <div
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px', borderRadius: 12, marginBottom: 8, border: `1.5px solid ${selectedAddress === addr.id ? '#ff6b35' : '#eee'}`, background: selectedAddress === addr.id ? '#fff9f6' : 'white', cursor: 'pointer' }}
              >
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: 16, marginBottom: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            {[
              ...
              <div
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px', borderRadius: 12, marginBottom: 8, border: `1.5px solid ${paymentMethod === method.id ? '#ff6b35' : '#eee'}`, background: paymentMethod === method.id ? '#fff9f6' : 'white', cursor: 'pointer' }}
              >
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 12 }}>Order Summary</div>
            {cart.map(item => (
            ...
            <div style={{ borderTop: '1px solid #eee', marginTop: 8, paddingTop: 8, display: 'flex', justifyContent: 'space-between', fontSize: 15, fontWeight: 700 }}>
      ...
      <div style={{ position: 'fixed', bottom: 70, left: 0, right: 0, padding: '12px 16px', background: 'white', borderTop: '1px solid #eee' }}>
        {step === 'cart' ? (
          <button
            ...
            style={{ width: '100%', background: cart.length === 0 ? '#ccc' : '#ff6b35', color: 'white', border: 'none', borderRadius: 12, padding: '16px', fontSize: 16, fontWeight: 600, cursor: cart.length === 0 ? 'not-allowed' : 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
          >
          ...
          <button
            onClick={placeOrder}
            disabled={loading}
            style={{ width: '100%', background: loading ? '#ccc' : '#ff6b35', color: 'white', border: 'none', borderRadius: 12, padding: '16px', fontSize: 16, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer' }}
          >
